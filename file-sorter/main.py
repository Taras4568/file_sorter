...
def create_folder(path:str, extension:str) ->str:
    # створює папку назва якої співпадає з назвою розшипення файлу
    folder_name: str = extension[1:]#[1:] пропускаємо крапку в розширенні
...
def sort_files(source_path: str):
    #сортує файли за заданим шляхом
    #.walk проходить через всі файли
...
def remove_empty_folders(source_path: str):
    #topdown - починаємо з вкладених папок і рухаємось до головної папки
...
def main():
    ...
    if os.path.exists(path = user_input):
        ...
        print('Fikes sorted successfully')
